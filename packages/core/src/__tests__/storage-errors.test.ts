      ...
      app.use('*', async (c, next) => {
        c.env = {
          USER_STORAGE: {
            ...
            async put() { return { key: 'test', size: 0, uploaded: new Date(), etag: 'test' } },
    ...
    beforeEach(() => {
      app.use('*', async (c, next) => {
        c.env = {
          USER_STORAGE: {
            async list() { return { objects: [], truncated: false, delimitedPrefixes: [] } },
            async put() { throw new Error('File too large') },
            async get() { return null },
            async head() { return null },
            async delete() {}
          } as any,
  ...
  describe('HTTP Exception Propagation', () => {
    beforeEach(() => {
      const throwingBucket = {
        async list() { throw new HTTPException(503, { message: 'Service temporarily unavailable' }) },
        async put() { throw new HTTPException(507, { message: 'Insufficient storage' }) },
        async get() { throw new HTTPException(403, { message: 'Access denied' }) },
        async head() { throw new HTTPException(429, { message: 'Rate limit exceeded' }) },
        async delete() { throw new HTTPException(409, { message: 'Conflict during deletion' }) }
      }
