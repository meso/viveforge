...
adminOAuth.put('/providers/:provider', async (c) => {
  try {
    ...
    // Validate provider
    const supportedProviders = ['google', 'github', 'facebook', 'linkedin', 'twitter', 'apple', 'microsoft', 'discord', 'slack']
...
adminOAuth.get('/supported-providers', async (c) => {
  try {
    ...
    },
    {
      ...
      setup_instructions: 'Go to GitHub Settings → Developer settings → OAuth Apps → New OAuth App'
    },
    {
      provider: 'facebook',
      name: 'Facebook',
