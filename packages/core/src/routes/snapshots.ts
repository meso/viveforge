...
snapshots.use('*', async (c, next) => {
  ...
  const tableManager = new TableManager(env.DB, env.SYSTEM_STORAGE)
...
snapshots.get('/compare/:id1/:id2', async (c) => {
  try {
    ...
    const added = schemas2.filter((s2: any) => 
      !schemas1.find((s1: any) => s1.name === s2.name)
    )
    const removed = schemas1.filter((s1: any) => 
      !schemas2.find((s2: any) => s2.name === s1.name)
    )
